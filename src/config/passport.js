...
export default function (passport) {
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          ...
          if (user) {
            ...
            const usernameBase = profile.email.split('@')[0].replace(/[^a-zA-Z]/g, '').toLowerCase();

            const newUser = new User({
              userId: `${usernameBase}_${new Date().getTime()}`,
              username: `${usernameBase}_${new Date().getTime()}`,
              email: profile.email,
              password: randomString,
  ...
  passport.use(
    ...
    new GoogleStrategy.Strategy(
      {
        clientID: process.env.GOOGLE_CLIENT_ID2 || "1234567",
        clientSecret: process.env.GOOGLE_CLIENT_SECRET2 || "1234567",
        callbackURL: '/api/v1/auth/message/google/callback'
      },
      async (accessToken, refreshToken, profile, done) => {
        try {
          const user = await User.findOne({ provider_id: profile.id }).populate('friends', 'profilePicture username firstname lastname');
          if (user) {
            return done(null, user);
          } else {
            const randomString = Math.random().toString(36).substring(2);
